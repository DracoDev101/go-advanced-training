...
- 设计 timeout budget。
- 正确使用 retry 与 backoff。
- 理解 circuit breaker 与 rate limit。
- 避免重试风暴。
...
- 本节关键词：**timeout、retry、backoff、circuit breaker、rate limit、bulkhead**。
...
Go 的工程化实践强调组合胜过继承、显式胜过隐式、可读胜过炫技。对于 **可靠性模式**，设计时应先问：
