...
- Redis 是高性能内存数据结构服务，不是关系数据库替代品。
- 缓存提升读取性能，但引入一致性和失效问题。
- TTL jitter 可缓解集中失效。
- 分布式锁只能在明确边界内使用，不能替代事务。
...
对于 `Redis 使用边界`，不要只记 API 或术语，而要理解它对以下方面的影响：
