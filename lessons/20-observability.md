...
- 可观测性不是加日志，而是让系统在故障时可解释。
- metrics 看趋势和告警，logs 看离散事件，traces 看请求链路。
- 高基数标签会拖垮指标系统。
- readiness 表示能否接流量，liveness 表示是否需要重启。
...
1. logs、metrics、traces 分别回答什么问题？
2. 什么是 high-cardinality label？
3. 如何把 trace id 注入日志？
4. health/readiness 区别是什么？
5. pprof 如何安全暴露？
...
对于 `Observability`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 日志字段稳定、短名、低冗余。
- 指标 label 不放 user_id/job_id 等高基数字段。
- 错误日志包含 operation 和 correlation id。
- pprof endpoint 要限制访问。
