...
- HTTP 服务不是只写 handler，还要设计生命周期、错误、观测和背压。
- server/read/write/idle timeout 都有意义。
- graceful shutdown 要停止接收新请求并等待 in-flight 收敛。
- 错误响应是 API contract 的一部分。
...
1. 生产 HTTP 服务需要哪些 timeout？
2. middleware 顺序如何设计？
3. graceful shutdown 如何确保请求收敛？
4. 错误响应 schema 如何稳定？
5. 幂等、分页、限流在哪里处理？
...
对于 `HTTP API 生产实践`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 统一 access log 和 error response。
- handler 只做解析、校验、调用 service、映射响应。
- 公共 API 默认 HTTP/JSON。
- 长期运行请求要支持 cancel。
