...
完成本课后，学习者应该能够掌握：`net/http.Server` 的 `ReadHeaderTimeout`、`ReadTimeout`、`WriteTimeout`、`IdleTimeout`；middleware 顺序；request body limit、JSON decoder、validation、error response schema；graceful shutdown。
...
生产级 HTTP 的核心不是路由库，而是超时、边界、错误语义、观测字段和关闭协议。
...
错误映射：domain error -> transport status；不要让 repository 直接决定 HTTP code。
...
配套实验目录：`labs/15-http-api-production/`
