...
- benchmark 必须控制输入、避免编译器优化和外部噪音。
- ns/op、B/op、allocs/op 要一起看。
- pprof 用于定位热点，不直接告诉你业务答案。
- 优化前后都要用同样基准验证。
...
1. 如何写可靠 benchmark？
2. benchmem 指标如何解读？
3. CPU profile 与 heap profile 看什么？
4. mutex/block profile 何时启用？
5. 如何避免 benchmark 误导？
...
对于 `Benchmark 与 pprof`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 性能 PR 必须包含 benchmark 前后对比。
- 生产慢请求先看 metrics 再 pprof。
- 不要在无 profile 证据时做微优化。
- profiling endpoint 需要访问控制。
