...
- gRPC 适合内部服务、高性能 RPC 和 streaming。
- proto 字段编号一旦发布不能随意复用。
- deadline 是 gRPC 可靠性的核心约束。
- interceptor 是 logging、metrics、auth、recovery 的统一入口。
...
1. protobuf 如何保持兼容？
2. deadline 与 metadata 如何传播？
3. interceptor 用来做什么？
4. status code 如何映射领域错误？
5. streaming 如何处理背压？
...
- deadline 是 gRPC 可靠性的核心约束。
- interceptor 是 logging、metrics、auth、recovery 的统一入口。
- gRPC 错误必须使用 status code 和稳定 detail。
...
对于 `gRPC 生产实践`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 内部服务优先 gRPC，外部浏览器/API 优先 HTTP JSON。
- 不要删除或复用已发布 field number。
- 服务端必须尊重 ctx.Done。
- 为本地调试开启 reflection，生产按安全策略决定。
