...
- channel 用于传递 ownership、事件和结果，不是所有共享状态的默认解。
- 无缓冲 channel 同步发送与接收；有缓冲 channel 引入队列和背压边界。
- 只有发送方或拥有者应该 close channel。
- nil channel 可用于动态禁用 select 分支。
...
1. channel 内部如何管理发送和接收？
2. buffered channel 是队列还是锁？
3. close 的语义是什么？
4. select 是否公平？
5. 什么时候该用 mutex 而不是 channel？
...
对于 `Channel 原理与使用边界`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
```text
labs/07-channel-internals-boundaries/
  README.md
  go.mod
  main.go 或 *_test.go
```
...
go test ./...
go test -race ./...
go test -run TestDeadlock -timeout=2s
```
...
- worker pool 适合 channel 分发任务。
- 共享 map/cache 用 mutex 更直接。
- 关闭 channel 要有唯一所有者。
- buffer 大小是系统设计参数，不是随便填。
