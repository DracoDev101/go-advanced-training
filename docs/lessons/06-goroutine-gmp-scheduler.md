...
- goroutine 是用户态调度的轻量执行单元，不是免费资源。
- P 是执行 Go 代码所需的调度上下文，GOMAXPROCS 决定 P 数量。
- Go scheduler 通过 work stealing 和 netpoller 提升并发效率。
- goroutine leak 在生产中比普通内存泄漏更隐蔽。
...
对于 `Goroutine 与 GMP 调度模型`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
```text
labs/06-goroutine-gmp-scheduler/
  README.md
  go.mod
  main.go 或 *_test.go
```
...
- 不要无界创建 goroutine。
- 每个 goroutine 都应有明确退出路径。
- I/O 密集服务关注 timeout 和 cancellation。
- CPU 密集任务关注 GOMAXPROCS、队列和背压。
