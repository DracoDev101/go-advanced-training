...
对于 `编译、链接与二进制部署`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 二进制注入 version/commit/build time。
- 服务镜像使用 distroless 或 scratch。
- 需要 DNS/证书/时区时注意 scratch 限制。
- CI 中固定 Go 版本并缓存 module。
