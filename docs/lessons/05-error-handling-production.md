...
- 错误路径是系统设计的一部分，必须显式。
- 底层错误要包装上下文，边界层要映射语义。
- errors.Is/As 依赖正确 wrapping。
- panic 只用于不可恢复程序错误或框架边界保护。
...
1. 为什么 Go 使用 error as value？
2. sentinel error 和 custom error 如何选择？
3. 什么时候 wrapping，什么时候 mapping？
4. panic/recover 边界在哪里？
5. HTTP/gRPC 如何映射领域错误？
...
对于 `Error Handling 生产实践`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
```text
labs/05-error-handling-production/
  README.md
  go.mod
  main.go 或 *_test.go
```
...
- repository 不直接返回 HTTP status。
- service 层定义领域错误，transport 层映射协议错误。
- 外部依赖错误要保留上下文但避免泄漏敏感信息。
- middleware recover 只兜底，不替代正常错误处理。
