...
完成本课后，学习者应该能够掌握：timeout、retry、circuit breaker、bulkhead、rate limit、backpressure、graceful degradation；retry amplification；幂等是 retry 的前提；可靠性策略必须有预算。
...
可靠性不是永远成功，而是在失败不可避免时限制影响范围，并让系统可恢复。

---

## 硬核要点

重试规则：只重试 transient error；指数退避 + jitter；总超时预算内重试；所有写操作必须幂等或可去重。Circuit breaker 保护下游，bulkhead 隔离资源池，backpressure 让上游知道系统已满。
...
配套实验目录：`labs/21-reliability-patterns/`

```bash
cd labs/21-reliability-patterns
go test ./...
go test -race ./...
