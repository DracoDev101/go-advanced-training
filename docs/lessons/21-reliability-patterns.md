...
- 可靠性设计的核心是限制故障传播。
- 所有外部调用必须有 timeout。
- retry 必须有上限、backoff、jitter 和幂等前提。
- bulkhead 用资源隔离防止单点依赖拖垮全局。
...
1. timeout、retry、circuit breaker 分别解决什么？
2. retry 为什么会放大故障？
3. bulkhead 如何隔离资源？
4. backpressure 如何设计？
5. 如何做 graceful degradation？
...
对于 `可靠性模式`，不要只记 API 或术语，而要理解它对以下方面的影响：
