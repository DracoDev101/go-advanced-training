...
- slice 是 header，底层数组才保存数据。
- append 是否共享底层数组取决于 cap 与扩容。
- 小 subslice 可能持有大数组，生产中要 copy 截断生命周期。
- Go map 为性能设计，不提供内置并发安全。
...
1. slice header 包含什么？
2. append 什么时候会影响原 slice？
3. subslice 为什么可能造成内存保留？
4. map 为什么不能并发读写？
5. string 与 []byte 转换成本在哪里？
...
- 小 subslice 可能持有大数组，生产中要 copy 截断生命周期。
- Go map 为性能设计，不提供内置并发安全。
- string 不可变，频繁拼接应使用 strings.Builder。
...
对于 `Slice、Map、String 底层结构`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
```text
labs/03-slice-map-string-internals/
  README.md
  go.mod
  main.go 或 *_test.go
```
...
- 返回大 buffer 的小切片前考虑 copy。
- JSON API 中区分 nil slice 与 empty slice。
- 普通并发 map 用 map+RWMutex；不要默认 sync.Map。
- 热点字符串拼接用 Builder 并 benchmark。
