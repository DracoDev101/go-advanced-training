...
- Go GC 优先降低尾延迟，而不是追求最高吞吐。
- GC 成本主要来自活跃对象数量和分配速率。
- 减少 allocation 往往比调 GOGC 更有效。
- GOMEMLIMIT 可以约束进程内存目标。
...
1. Go GC 为什么是并发 GC？
2. 三色标记是什么？
3. write barrier 解决什么问题？
4. GOGC 控制什么？
5. 为什么 allocation rate 比 heap size 更关键？
...
对于 `Go GC 原理`，不要只记 API 或术语，而要理解它对以下方面的影响：
...
- 用 metrics 观察 heap、alloc rate、GC pause。
- 热点路径减少临时对象和 interface boxing。
- 大对象生命周期要清晰，避免缓存无界增长。
- 调参前先用 pprof 找 allocation hotspot。
