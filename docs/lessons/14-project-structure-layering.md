...
完成本课后，学习者应该能够掌握：`cmd/`、`internal/`、`pkg/` 的真实边界；transport/application/domain/infrastructure 依赖方向；interface 定义在使用方；config/logger/db 初始化只在 composition root 完成。
...
项目结构的价值不是“看起来像大厂”，而是让依赖方向稳定、业务规则可测试、基础设施可替换。
...
```text
cmd -> transport -> application -> domain
application -> ports(interface)
infrastructure -> ports implementation
```

反例：handler 直接写 SQL、domain 引入 `*sql.DB`、repository 返回 HTTP status。
...
配套实验目录：`labs/14-project-structure-layering/`

```bash
cd labs/14-project-structure-layering
go test ./...
go test -race ./...
