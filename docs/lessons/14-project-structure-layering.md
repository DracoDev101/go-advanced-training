...
- 设计可维护 Go 项目结构。
- 区分 cmd/internal/pkg 的边界。
- 建立 handler-service-repository 分层。
- 避免循环依赖和过早抽象。
...
- 本节关键词：**project structure、layering、dependency、internal、repository、service**。
...
Go 的工程哲学偏向直接、可读、可组合。对于 **项目结构与分层设计**，不要把问题理解成“选一个库”或“套一个模式”，而要从系统边界出发：
