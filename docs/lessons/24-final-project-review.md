...
完成本课后，学习者应该能够掌握：架构 review：边界、依赖方向、状态机和一致性；代码 review：错误处理、context、测试、并发安全；性能 review：benchmark、profile、容量预算；运维 review：日志、指标、trace、部署、回滚、故障演练。
...
最终 Review 不是演示功能，而是证明系统在失败、并发、容量和演进压力下仍然可解释、可观测、可恢复。

---

## 硬核要点

Review checklist：是否所有外部调用都有 timeout？是否所有 retry 都有幂等保护？是否能从 job_id 查到完整日志和 trace？worker crash 后 job 如何恢复？DB migration 失败如何回滚？P99 升高时先看哪三个 dashboard？
...
配套实验目录：`labs/24-final-project-review/`

```bash
cd labs/24-final-project-review
go test ./...
go test -race ./...
