# Lesson 19：消息队列、事件与幂等

## 学习目标

完成本课后，学习者应该能够掌握：at-most-once/at-least-once/exactly-once 的现实含义；retry、DLQ、backoff、poison message；idempotent consumer；outbox pattern 与 schema evolution。
...
消息队列默认带来的是解耦和削峰，不是自动可靠；可靠性来自确认、重试、幂等和可观测。
...
配套实验目录：`labs/19-message-queue-events/`

```bash
cd labs/19-message-queue-events
go test ./...
go test -race ./...
