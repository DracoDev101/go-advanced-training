...
func TestProcessRecordsEvents(t *testing.T) {
    var r Recorder
    if err := Process(context.Background(), &r, SampleEvents()); err != nil {
        t.Fatalf("Process returned error: %v", err)
    }
    if r.Count() != int64(len(SampleEvents())) {
        t.Fatalf("count got %d", r.Count())
    }
    if len(r.Events()) != len(SampleEvents()) {
        t.Fatalf("events not recorded")
    }
}
...
func TestProcessWithTimeout(t *testing.T) {
    var r Recorder
    if err := ProcessWithTimeout(context.Background(), &r, SampleEvents(), time.Second); err != nil {
        t.Fatalf("ProcessWithTimeout returned error: %v", err)
    }
}

func TestTopic(t *testing.T) {
    if Topic() == "" { t.Fatal("topic should not be empty") }
}
